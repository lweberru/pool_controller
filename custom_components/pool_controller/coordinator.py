...
class PoolControllerDataCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        try:
            ...
            pv_gen = self._get_float(conf.get(CONF_PV_SURPLUS_SENSOR))
            
            # WE/Feiertag (Heute Sonntag -> Aktiv)
            ...
            ph_minus = max(0, round((ph_val - 7.2) * 100 * vol_f)) if ph_val and ph_val > 7.2 else 0
