...
class PoolMainSwitch(PoolBaseSwitch):
    ...
    async def async_turn_on(self, **kwargs): await self.hass.services.async_call("switch", "turn_on", {"entity_id": self.coordinator.entry.data.get(CONF_MAIN_SWITCH)})
    async def async_turn_off(self, **kwargs): await self.hass.services.async_call("switch", "turn_off", {"entity_id": self.coordinator.entry.data.get(CONF_MAIN_SWITCH)})

class PoolAuxSwitch(PoolBaseSwitch):
    _attr_translation_key = "aux_heating"
    @property
    def is_on(self): return self.coordinator.data.get("should_main_on") # Vereinfachte Logik
    async def async_turn_on(self, **kwargs): await self.hass.services.async_call("switch", "turn_on", {"entity_id": self.coordinator.entry.data.get(CONF_AUX_HEATING_SWITCH)})

class PoolBathingSwitch(PoolBaseSwitch):
    _attr_translation_key = "bathing_mode"
    @property
